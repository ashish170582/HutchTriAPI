...
public class AmazonS3Manager {
            ...
            synchronized (AmazonS3Manager.class) {
                if (instance == null) {
                    try {
                        ...
                        String accessKey = "AKIAJ6MQWJQT7WIIMG2A";
                        String secretKey = "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";
    ...
    public void put(String bucket, String key, String filePath, ObjectMetadata metadata) {
        ...
        try {
            ...
            System.out.println("Uploaded to S3: '" + filePath + "' with key '" + key
                    + "'");
    ...
    public S3Object get(String bucket, String key) {
        ...
        try {
            System.out.println("Finding an s3object...");
    ...
    public S3ObjectSummary getLatestBuildArtifact(String bucket, String key, Pattern pattern) {
        ...
        do {
            System.out.println("looking for s3 artifact using iteration #" + i);

            for (S3ObjectSummary obj : objBuilds.getObjectSummaries()) {
                System.out.println("Existing S3 artifact: " + obj.getKey());
        ...
        if (latestBuild == null) {
            System.out.println("Unable to find S3 build artifact by pattern: " + pattern);
        } else {
            System.out.println("latest artifact: " + latestBuild.getKey());
        }
        return latestBuild;
    }
     ...
     * @param key (example: android/apkFolder/ApkName.apk)
     * @param file (local file name)
     * @param pollingInterval (polling interval in sec for S3 download status
     * determination)
     */
    public void download(final String bucketName, final String key, final File file, long pollingInterval) {
        System.out.println("App will be downloaded from s3.");
        System.out.println(String.format("[Bucket name: %s] [Key: %s] [File: %s]", bucketName, key, file.getAbsolutePath()));
        DefaultAWSCredentialsProviderChain credentialProviderChain = new DefaultAWSCredentialsProviderChain();
        TransferManager tx = new TransferManager(
                credentialProviderChain.getCredentials());
        Download appDownload = tx.download(bucketName, key, file);
        try {
            System.out.println("Transfer: " + appDownload.getDescription());
            System.out.println("	State: " + appDownload.getState());
            System.out.println("	Progress: ");
            // You can poll your transfer's status to check its progress
            while (!appDownload.isDone()) {
                System.out.println("		transferred: " + (int) (appDownload.getProgress().getPercentTransferred() + 0.5) + "%");
	 ...
	 * line at a time and display. System.out.println("File content is: ");
	 * BufferedReader reader = new BufferedReader(new InputStreamReader(input));
	 * while (true) { String line = null; try { line = reader.readLine(); }
	 * catch (IOException e) { System.out.println("Failed to read file", e); } if
	 * (line == null) break;
	 *
