...
@Component
public class DataBaseProcedures_bkp {
    ...
    public AppVersionInfoData getAppVersionInfo(String countryCode, String operatingSystem, String applicationVersion) throws SQLException {
        AppVersionInfoData appVersionInfoData = null;
        MySQL MySQL = new MySQL();
        try {

          
            rs = MySQL.prepareCall("{call `ApplicationVersionInformation`('" + countryCode + "','" + operatingSystem + "','" + applicationVersion + "')}");
            if (rs != null) {
                while (rs.next()) {
    ...
    @SuppressWarnings("finally")
	public String getFollowArtist(RequestParameter reqParam) throws SQLException {
    ...
    @SuppressWarnings("finally")
	public List<ContainerBean> getKaraokeContainerList(RequestParameter reqParam, DeviceInformation devInfo) throws SQLException {
	...
	@SuppressWarnings("finally")
	public List<SubscriptionPackage> getSubscriptionPackageList(RequestParameter reqParam) throws SQLException {
	...
	@SuppressWarnings("finally")
	public List<DownloadPackage> getDownloadPackageList(RequestParameter reqParam) throws SQLException {
		List<DownloadPackage> lst = new ArrayList<DownloadPackage>();
		MySQL MySQL = new MySQL();
		try {
			
			rs = MySQL.prepareCall(
	...
	public int requestDirectSubscription(RequestParameter reqParam) throws SQLException {
		...
		try {
			...
			rs = MySQL.prepareCall("{CALL `DIRECT_ZA_BILLING`(" + reqParam.getCountryId() + ","
	...
	@SuppressWarnings("finally")
	public int requestSubscription(RequestParameter reqParam) throws SQLException {
		int responseCode = 110;
		MySQL MySQL = new MySQL();
		try {
			...
			if (reqParam.getOperatorId() != -1) {
				 ...
				 rs = MySQL.prepareCall("{CALL `Subscribe`(" + reqParam.getCountryId() + ","
	...
	@SuppressWarnings("finally")
	public int requestUnSubscription(RequestParameter reqParam) throws SQLException {
		int responseCode = 110;
		MySQL MySQL = new MySQL();
		try {
		
			rs = MySQL.prepareCall("{CALL `Unsubscribe`(" + reqParam.getUserId() + ")}");
			if (rs != null) {
				while (rs.next()) {
					responseCode = rs.getInt("code");
				}
	...
	@SuppressWarnings("finally")
	public int appInstallViaShareClickTraking(RequestParameter reqParam) throws SQLException {
		int resultCode = 317;
		MySQL MySQL = new MySQL();
		try {

			
			 rs = MySQL.prepareCall("{call `appInstallViaShareClickTraking`('" + reqParam.getEventType()
	...
	@SuppressWarnings("finally")
	public AudioQualityConfiguration getAudioQualityConfiguration(RequestParameter reqParam) throws SQLException {
		...
		try {
			 ...
			 rs = MySQL.prepareCall("{CALL `AudioQualityConfiguration`('" + reqParam.getCountryCode() + "',"
	...
	public UserInfoData signIn(RequestParameter reqParam) {
		...
		try {
			...
			MySQL mysql = new MySQL();
			ResultSet rs = mysql.prepareCall("{CALL `SignIn`(" + reqParam.getCountryId()
					+ "," + reqParam.getSourceId() + ",'" + reqParam.getOperatingSystem() + "','"
					+ reqParam.getOperatingSystemVersion() + "','" + reqParam.getDeviceModel() + "','"
					+ reqParam.getDeviceId() + "','" + reqParam.getDevicePin() + "'," + reqParam.getViaId() + ", '"
					+ reqParam.getApplicationVersion() + "' ,'" + reqParam.getUserName().replaceAll("'", "''") + "','"
					+ reqParam.getMsisdn() + "','" + reqParam.getEmailAddress().replaceAll("'", "''") + "','"
					+ reqParam.getPassword().replaceAll("'", "''") + "'," + reqParam.getGenderId() + ",'"
					+ this.getFinalRedirectedUrl(new StringBuilder().append("http://graph.facebook.com/")
							.append(reqParam.getFbProfileId()).append("/picture?type=large").toString())
							.replaceAll("'", "''")
					+ "')}");
			if (rs != null) {
				while (rs.next()) {
...
//
			/*
			 * userinfodata = new
			 * UserInfoData(Integer.parseInt(rs.get("user_id").toString()),
			 * rs.get("user_promo_code").toString(), rs.get("user_name").toString(),
			 * rs.get("mobile_number").toString(), rs.get("email_address").toString(),
			 * Integer.parseInt(rs.get("gender_id").toString()),
			 * rs.get("password").toString(), rs.get("image_url").toString(),
			 * Integer.parseInt(rs.get("user_type_id").toString()),
			 * Integer.parseInt(rs.get("user_status").toString()),
			 * Integer.parseInt(rs.get("action_id").toString()),
			 * Integer.parseInt(rs.get("registered").toString()),
			 * Integer.parseInt(rs.get("mobile_number_verification").toString()),
			 * rs.get("billing_mobile_number").toString(),
			 * rs.get("dialing_code").toString(),
			 * Integer.parseInt(rs.get("mobile_number_length").toString()), new
			 * SubscribedPackage( Integer.parseInt(rs.get("package_id").toString()),
			 * rs.get("package_name").toString(), rs.get("package_price").toString(),
			 * rs.get("package_validity_period").toString(),
			 * rs.get("package_start_date").toString(),
			 * rs.get("package_end_date").toString(), rs.get("android_token").toString(),
			 * rs.get("ios_token").toString(),
			 * Boolean.parseBoolean(rs.get("optScreenVisibility").toString()) )
			 * 
			 * );
			 */
			...
			e.printStackTrace();
			userinfodata = null;
			System.out.println("Exception in HutchTriBeatzMainServlet.signIn(RequestParameter reqParam) - "
					+ e.getMessage() + "\t {CALL `SignIn`(" + reqParam.getCountryId() + "," + reqParam.getSourceId()
					...
					+ reqParam.getUserName().replaceAll("'", "''") + "','"
...
//						.addValue("inProfilePictureUrl", getFinalRedirectedUrl("http://graph.facebook.com/" + reqParam.getFbProfileId() + "/picture?type=large").replaceAll("'", "''"));
//								
//			
//			  
